    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fade-in pb-10">
      <div className="lg:col-span-4 bg-slate-800 p-6 rounded-2xl border border-slate-700 h-fit sticky top-24">
        ...
        <div className="space-y-4">
          ...
          <input type="text" placeholder="URL Imagen (https://...)" className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white focus:border-blue-500 outline-none transition-colors" value={newProduct.imageUrl || ''} onChange={e => setNewProduct({...newProduct, imageUrl: e.target.value})} />
              ...
              <tr key={q.id} className="hover:bg-slate-700/30 transition-colors">
                ...
                <td className="px-4 py-4">
                  <select 
                    ...
                    className={`text-xs font-bold px-3 py-1.5 rounded-full border bg-opacity-10 border-opacity-20 outline-none cursor-pointer transition-colors ${
                      q.status === 'Pendiente' ? 'bg-yellow-500 text-yellow-400 border-yellow-500' :
    ...
    <div className="flex flex-col lg:flex-row min-h-screen pt-20 bg-slate-900">
      ...
      <div className="flex-1 lg:ml-64 p-4 lg:p-8 bg-slate-900 pb-24 min-h-screen">
        ...
        <div className="bg-slate-900 rounded-3xl">
           {currentAdminView === AdminView.DASHBOARD && <DashboardView />}
           {currentAdminView === AdminView.PRODUCTS && <ProductsView />}
           {currentAdminView === AdminView.QUOTES && <QuotesView />}
           {currentAdminView === AdminView.CONTENT && <ContentView />}
        </div>
      </div>
    </div>
  );
